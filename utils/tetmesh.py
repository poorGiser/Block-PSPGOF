...
def _unbatched_marching_tetrahedra(vertices, tets, sdf, scales):
    ...
    #转移数据防止oom
    # vertices = vertices.to(device_b)
    # tets = tets.to(device_b)
    # sdf = sdf.to(device_b)
    # scales = scales.to(device_b)
    # device = device_b
    ...
    if tets.shape[0] > chunk_size:
        ...
        # merged_verts = (merged_verts[0].to(device_a),merged_verts[1].to(device_a))
        # merged_scales = merged_scales.to(device_a)
        # merged_faces = merged_faces.to(device_a)
        # merged_verts_ids = merged_verts_ids.to(device_a)
...
def marching_tetrahedra(vertices, tets, sdf, scales,batch_size=None):
    ...
    if batch_size is None:
        ...
        # pass
        list_of_outputs = []
        verts_num = vertices.shape[1]
        batch_size = 1024 * 4
        for i in range(0,verts_num,batch_size):
            if i + batch_size < verts_num:
                ter = _unbatched_marching_tetrahedra(vertices[0,i:i + batch_size,:][None], tets, sdf[0,i:i + batch_size][None], scales[0,i:i + batch_size][None])
            else:
                ter = _unbatched_marching_tetrahedra(vertices[0,i:verts_num,:][None], tets, sdf[0,i:verts_num][None], scales[0,i:verts_num][None])
            list_of_outputs.append(ter)
