...
@torch.no_grad()
def marching_tetrahedra_with_binary_search(model_path, name, iteration, views, gaussians, pipeline, background, kernel_size, filter_mesh : bool, texture_mesh : bool,extend = None,par_idx = None,plane_iso=2):
    ...
    '''
    points, points_scale = gaussians.get_tetra_points(extend=extend,plane_iso=plane_iso)
    
    # device = torch.device("cuda:0")
    # point_cloud = o3d.io.read_point_cloud("/home/chenyi/gaussian-opacity-fields-main/gaussian-opacity-fields-main/output/power_station/block6/temp/depth_points.ply",print_progress = True)
    # points = torch.tensor(np.asarray(point_cloud.points),device=device,dtype=torch.float32)
    # points_scale = torch.ones([points.shape[0],1],device=device,dtype=torch.float32) * 100
    ...
    # points, points_scale = gaussians.get_tetra_points_from_depth(depths,views,extend)
    ...
    if par_idx is None:
        ...
        if os.path.exists(os.path.join(render_path, f"cells_{par_idx}.pt")):
            print("load existing cells")
            cells = torch.load(os.path.join(render_path, f"cells_{par_idx}.pt"))
            ...
            cells = cpp.triangulate(points)#Delaunay三角剖分生成四面体
            # we should filter the cell if it is larger than the gaussians
            torch.save(cells, os.path.join(render_path, f"cells_{par_idx}.pt"))
    # evaluate alpha
    ...
    # batch_size = 1024 * 4
    # for i in range(0,verts_num,batch_size):
    ...
    for step in range(n_binary_steps):#二分搜索
        ...
        if par_idx is None:
            ...
            ms.save_current_mesh(os.path.join(render_path, f"mesh_binary_search_{step}.ply"))
            print("Done!")
        else:
           return mesh
    

def extract_mesh(dataset : ModelParams, iteration : int, pipeline : PipelineParams, filter_mesh : bool, texture_mesh : bool,partitioning = False,partitioning_num = 2,plane_iso=2):
    ...
    if not partitioning:
        with torch.no_grad():
            ...
            marching_tetrahedra_with_binary_search(dataset.model_path, "test", iteration, cams, gaussians, pipeline, background, kernel_size, filter_mesh, texture_mesh,extend=origin_extend,plane_iso=plane_iso)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-partitioning", type=bool, default = True)
    ...
    extract_mesh(model.extract(args), args.iteration, pipeline.extract(args), args.filter_mesh, args.texture_mesh,partitioning=args.partitioning,partitioning_num=args.partitioning_num,plane_iso=args.plane_iso)