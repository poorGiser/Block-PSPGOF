	...
	)
{
	...
	// create the ray
	float2 ray = { (pixf.x - W/2.) / focal_x, (pixf.y - H/2.) / focal_y };//内参矩阵，像素坐标到相机坐标
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float3 cam_pos_local = {view2gaussian_j[12], view2gaussian_j[13], view2gaussian_j[14]};//高斯坐标系下的cam_pos
			float3 ray_local = transformPoint4x3_without_t(ray_point, view2gaussian_j);//高斯坐标系下的像素坐标

			// scale the ray_local and cam_pos_local
			//论文公式2，3
			double3 ray_local_scaled = { ray_local.x / scale_j.x, ray_local.y / scale_j.y, ray_local.z / scale_j.z };//rg
			double3 cam_pos_local_scaled = { cam_pos_local.x / scale_j.x, cam_pos_local.y / scale_j.y, cam_pos_local.z / scale_j.z };//og
			...
			double min_value = -(BB/AA) * (BB/4.) + CC;//t取到-B/A时的指数部分
			...
			//TODO: What is max_t
			const float max_t = t;
			const float mapped_max_t = (FAR_PLANE * max_t - FAR_PLANE * NEAR_PLANE) / ((FAR_PLANE - NEAR_PLANE) * max_t);//一个非线性的映射
	...
	if (inside)
	{
		...
		// depth and alpha
		out_color[DEPTH_OFFSET * H * W + pix_id] = C[CHANNELS * 2];//最大深度
		out_color[ALPHA_OFFSET * H * W + pix_id] = C[CHANNELS * 2 + 1];//不透明度
