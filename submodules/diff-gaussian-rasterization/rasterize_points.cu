	...
	)
{
  ...
  //alpha depth and mode depth
	...
	// if(rt_depths){
	// 	printf("need return alpha depth and mode depth!");
	// }else{
	// 	printf("do not return depth!");
	// }
	torch::Tensor point_list;
	if(rt_depths)//record points info to remove 
  ...
  return std::make_tuple(rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer,mode_id,modes,alpha_depth,out_means2D,out_conic_opacity,point_list);
}
